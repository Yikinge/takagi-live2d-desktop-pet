...
pub unsafe fn convert(
    ...
    if let Some(event_type) = option_type {
        return Some(Event {
            event_type,
            time: SystemTime::now(),
            // This application consumes semantic key zones only. Avoid asking
            // macOS to translate a key code through the active input method:
            // that would materialize private text we never need, and TIS input
            // source APIs trap when called from rdev's background event loop
